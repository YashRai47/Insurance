            ...
            if search_button and query:
                with st.spinner(f"🔍 Searching using {search_mode}..."):
                    ...
                    if search_result.get('success'):
                        ...
                        def format_record_block(record, index, styles):
                            elements = []

                            elements.append(
                                Paragraph(f"<b>Customer {index}</b>", styles["Heading3"])
                            )
                            ...
                            for key, value in record.items():
                                ...
                                if isinstance(value, dict):
                                    # Flatten dict nicely
                                    value = ", ".join(
                                        f"{k.title()}: {v}" for k, v in value.items()
                                    )
                        ...
                        def build_dynamic_table(data_list, title, styles):
                            elements = []
                            ...
                            if not data_list:
                                ...
                                elements.append(Paragraph("No relevant data available.", styles["Normal"]))
                                elements.append(Spacer(1, 24))
                                return elements

                            # Flatten records
                            flattened = [flatten_dict(item) for item in data_list]

                            # Collect headers that actually have values
                            header_value_count = {}
                            for item in flattened:
                                for k, v in item.items():
                                    if v not in ("", None, {}, []):
                                        header_value_count[k] = header_value_count.get(k, 0) + 1

                            # Keep only meaningful columns
                            headers = [
                                h for h, count in header_value_count.items()
                                if count > 0
                            ]
                            ...
                            readable_headers = [
                                h.replace("_", " ").replace("•", "-").title()
                                for h in headers
                            ]

                            table_data = [readable_headers]
                        ...
                        def generate_search_results_pdf(search_result, search_mode, query):
                            buffer = BytesIO()
                            ...
                            # Title
                            elements.append(Paragraph("Search Results Report", styles["Title"]))
                            elements.append(Spacer(1, 14))

                            # Meta
                            meta = f"""
                                <b>Search Query:</b> {query}<br/>
                                <b>Search Mode:</b> {search_mode}<br/>
                                <b>Generated On:</b> {datetime.now().strftime('%d %b %Y, %I:%M %p')}
                            """
                            ...
                            elements.append(Paragraph("Customer Results", styles["Heading2"]))
                            elements.append(Spacer(1, 12))

                            if not customers:
                                elements.append(Paragraph("No relevant customer data available.", styles["Normal"]))
                            else:
                                for idx, customer in enumerate(customers, 1):
                                    elements.extend(
                                        format_record_block(customer, idx, styles)
                                    )
                            ...
                            elements.append(Paragraph("Policy Results", styles["Heading2"]))
                            elements.append(Spacer(1, 10))

                            if not policies:
                                elements.append(Paragraph("No relevant policy data available.", styles["Normal"]))
                            else:
                                for idx, policy in enumerate(policies, 1):
                                    elements.extend(
                                        format_record_block(policy, idx, styles)
                                    )
                        ...
                        #     # Dynamic tables
                        #     elements.extend(build_dynamic_table(
                        #         search_result.get("customers", []),
                        #         "Customer Results",
                        #         styles
                        #     ))

                        #     elements.extend(build_dynamic_table(
                        #         search_result.get("policies", []),
                        #         "Policy Results",
                        #         styles
                        #     ))
                        ...
                        pdf_buffer = generate_search_results_pdf(
                        ...
                        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                        filename = f"search_results_{search_mode.replace(' ', '_').lower()}_{timestamp}.pdf"
                        ...
                        st.download_button(
                            label="📄 Download Search Results (PDF)",
                            data=pdf_buffer,
                            file_name=filename,
